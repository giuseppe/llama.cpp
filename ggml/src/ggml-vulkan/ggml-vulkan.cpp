...
#define IM2COL(bda) \
    ...
    const bool use_subgroups_ssm = device->subgroup_arithmetic && device->architecture != vk_device_architecture::AMD_GCN && device->vendor_id != VK_VENDOR_ID_INTEL;
    const uint32_t ssm_variant = use_subgroups_ssm ? 1 : 0;
